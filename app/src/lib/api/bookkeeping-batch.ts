...
export const bookkeepingBatchApi={
 bookkeepingWorkbench:(slug:string,input:{from:string;to:string;status?:string;bankAccountId?:number;partyId?:string;documentQuality?:"matched"|"missing";account?:string;vatTreatment?:string;dimension?:string;cursor?:number;limit?:number;search?:string})=>request<any>(`/api/companies/${encodeURIComponent(slug)}/bookkeeping-workbench?${new URLSearchParams(Object.entries(input).filter(([,value])=>value!==undefined).map(([key,value])=>[key,String(value)]))}`),

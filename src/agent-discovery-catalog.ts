...
export const AGENT_WORKFLOWS: readonly AgentWorkflow[] = [
  ...
  workflow({ id: "bank-reconciliation-batch", capabilityId: "bank-bookkeeping", title: "Bank import, matching and bookkeeping batch", intendedOutcome: "Import bank activity, inspect matches and apply only a hash-bound reviewed bookkeeping batch.", steps: [
    ...
    read("batch-dry-run", mcp("bookkeeping_batch_dry_run"), "Return the partitioned plan and plan hash.", { dependsOn: ["suggest-matches"], boundary: "dry-run", outputIdentities: ["planHash"] }),
    write("batch-apply", mcp("bookkeeping_batch_apply"), "Apply or resume the exact reviewed plan.", { dependsOn: ["batch-dry-run"], boundary: "irreversible", retryClass: "stable-key-resume", inputIdentities: ["runKey", "planHash"], outputIdentities: ["runId", "journalEntryIds"], uncertainOutcomeReadBack: mcp("journal_list"), canonicalRecords: ["bookkeeping batch runs", "journal entries", "bank reconciliations"] }),

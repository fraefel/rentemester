...
export const bankSpecs: CommandSpec[] = [
  ...
  { key:"bank legacy-binding-plan",usage:"bank legacy-binding-plan --company <path> --bank-account-id <n> --ledger-account <konto> --cutoff <YYYY-MM-DD>",description:"Read-only plan for the one permitted NULL→bank-ledger binding of an imported legacy bank account.",allowedFlags:["--company","--bank-account-id","--ledger-account","--cutoff"] },
  { key:"bank legacy-binding-apply",usage:"bank legacy-binding-apply --company <path> --bank-account-id <n> --ledger-account <konto> --cutoff <YYYY-MM-DD> --plan-hash <sha256> --principal user:<id>|service-account:<id> --idempotency-key <key> --confirm yes",description:"Applies exactly the reviewed initial bank binding; remapping is refused.",allowedFlags:["--company","--bank-account-id","--ledger-account","--cutoff","--plan-hash","--principal","--idempotency-key","--confirm"] },
  { key:"bank legacy-payable-backfill-plan",usage:"bank legacy-payable-backfill-plan --company <path> --purchase-journal-entry-id <n> --payment-journal-entry-id <n> --document-id <n> --bank-transaction-id <n>",description:"Read-only explicit-ID plan for an already-posted legacy payable and its payment; no amount matching.",allowedFlags:["--company","--purchase-journal-entry-id","--payment-journal-entry-id","--document-id","--bank-transaction-id"] },
  { key:"bank legacy-payable-backfill-apply",usage:"bank legacy-payable-backfill-apply --company <path> --purchase-journal-entry-id <n> --payment-journal-entry-id <n> --document-id <n> --bank-transaction-id <n> --plan-hash <sha256> --principal user:<id>|service-account:<id> --idempotency-key <key> --confirm yes",description:"Appends only the reviewed canonical payable/payment and audit evidence for exact IDs.",allowedFlags:["--company","--purchase-journal-entry-id","--payment-journal-entry-id","--document-id","--bank-transaction-id","--plan-hash","--principal","--idempotency-key","--confirm"] },

...
export const gdprSpecs: CommandSpec[] = [
  {
    ...
    inputNotes: [
      ...
      "fingerprint er sha256 af canonicalPayload; en Ed25519-signatur dækker præcis de samme UTF-8-bytes",
      "--sign-with-ed25519 kræver at backup-nøgleparret er genereret én gang via 'system backup --sign-with-ed25519'",
    ],
  },
  {
    key: "gdpr discover",
    usage: "gdpr discover --company <path> (--cvr <DK...> | --subject <id> | --name <text>)",
    description: "Subject-discovery: lister hvor en data-subject optræder pr. tabel (kunder, leverandører, bilag, banktransaktioner). Hurtigere end 'gdpr export', men append-only audit-logges og er derfor actor-gatet.",
    allowedFlags: ["--company", "--cvr", "--subject", "--name"],
    inputNotes: [
      "Den registrerede identificeres med --cvr/--subject og/eller --name",
      "Output er deterministisk og audit-loggable: byTable-tæller + tombstone-redigerede rækker pr. tabel",
      "Et navn der matcher flere strukturerede identiteter afvises; angiv CVR for at disambiguere",
  ...
  },
  {
    ...
    description: "Samler alle persondata Rentemester har om en kunde/leverandør i én indsigtsrapport med opbevaringsvurdering. Append-only audit-logges og er derfor actor-gatet.",
  ...
  {
    key: "gdpr erase",
    usage: "gdpr erase --company <path> (--cvr <DK...> | --name <text>)",
    ...
    inputNotes: [
      ...
      "Retention vurderes altid mod systemets aktuelle danske dato; slettekald accepterer ingen simuleret --as-of",
      "Sletning skrives som append-only tombstone; finansposteringer ændres ikke",
    ],
  },
  {
    key: "gdpr forget",
    usage: "gdpr forget --company <path> (--cvr <DK...> | --subject <id> | --name <text>) --after-retention-expiry",
    ...
    allowedFlags: ["--company", "--cvr", "--subject", "--name", "--after-retention-expiry"],
    inputNotes: [
      "Den registrerede identificeres med --cvr/--subject og/eller --name",

...
export function register(dispatch: CommandDispatch): void {
  ...
  dispatch.on("documents", "parse-pending", async (ctx) => {
    ...
    const db = openCommandDb(ctx); migrate(db); try { const ids = (db.query(`SELECT d.id FROM documents d WHERE d.mime_type='application/pdf' AND d.stored_path IS NOT NULL AND NOT EXISTS (SELECT 1 FROM document_pdf_parse_results p WHERE p.document_id=d.id) ORDER BY d.id LIMIT ?`).all(limit) as Array<{ id: number }>).map((x) => x.id); const parses = await parseRegisteredPdfBatch(db, ctx.companyRoot(), ids, { createdBy: ctx.cliActor ?? ctx.inferredMutationActor() ?? undefined, createdByProgram: "rentemester-cli" }); const failed = parses.filter((p: any) => !p.ok); ctx.emitResult({ ok: true, batch: { requested: ids.length, parsed: parses.length - failed.length, failed: failed.length, resume: failed.length ? { documentIds: failed.map((p: any) => p.documentId) } : null } }); } finally { db.close(); }
  });
  dispatch.on("documents", "parse-status", (ctx) => { const id = Number(ctx.arg("--document-id")); if (!Number.isInteger(id) || id <= 0) ctx.fatal("Missing required --document-id <n>"); const db = openLedgerReadOnly(companyPaths(ctx.companyRoot()).db); try { if (inspectOpenLedger(db).status !== "current") ctx.fatal("ledger is not ready for read-only inspection"); ctx.emitResult({ ok: true, parse: documentPdfParseStatus(db, id) }); } finally { db.close(); } });
  dispatch.on("documents", "parsed-text", (ctx) => { const id = Number(ctx.arg("--document-id")); const offset = Number(ctx.arg("--offset") ?? 0); const limit = Number(ctx.arg("--limit") ?? 10); if (!Number.isInteger(id) || id <= 0) ctx.fatal("Missing required --document-id <n>"); if (!Number.isInteger(offset) || offset < 0 || !Number.isInteger(limit) || limit < 1 || limit > 10) ctx.fatal("--offset >= 0 and --limit 1..10 are required"); const db = openLedgerReadOnly(companyPaths(ctx.companyRoot()).db); try { if (inspectOpenLedger(db).status !== "current") ctx.fatal("ledger is not ready for read-only inspection"); ctx.emitResult({ ok: true, ...documentPdfParsedText(db, id, offset, limit) }); } finally { db.close(); } });
}

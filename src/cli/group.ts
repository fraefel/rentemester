...
export function register(dispatch: CommandDispatch): void {
  ...
  dispatch.on("group", "disposition-status", (ctx) => {try{const workspace=resolveWorkspaceRoot(required(ctx,"--workspace")),db=openWorkspaceControlReadOnlyDb(workspace);try{const disposition=intercompanyDispositionStatus(db,workspace,required(ctx,"--disposition-id"),ctx.arg("--as-of")??undefined);ctx.emitResult(disposition?{ok:true,...disposition}:{ok:false,errors:["disposition not found"]});}finally{db.close();}}catch(error){ctx.emitResult({ok:false,errors:[error instanceof Error?error.message:"disposition status unavailable"]});}});
  dispatch.on("group", "settle-disposition", (ctx) => {if ((ctx.arg("--confirm")??"").trim().toLowerCase()!=="yes"){ctx.emitResult({ok:false,errors:["--confirm yes required to settle intercompany disposition"]});return;}try{const workspace=resolveWorkspaceRoot(required(ctx,"--workspace")),audit=actorInput(ctx),id=required(ctx,"--disposition-id"),reader=openWorkspaceControlReadOnlyDb(workspace),before=inspectIntercompanyDisposition(reader,id);reader.close();if(!before)throw new Error("disposition not found");assertCompanySlugsAuthorize(workspace,[before.disposition.left.companySlug,before.disposition.right.companySlug],audit.createdBy,"group settle-disposition");const evidence=JSON.parse(required(ctx,"--settlement-evidence-json"));const db=openWorkspaceControlDb(workspace);try{ctx.emitResult({ok:true,...settleIntercompanyDisposition(db,workspace,{dispositionId:id,payloadHash:required(ctx,"--payload-hash"),settlementEvidenceRecordIds:evidence,actor:audit.createdBy,principal:{kind:"user",id:audit.createdBy}})});}finally{db.close();}}catch(error){ctx.emitResult({ok:false,errors:[error instanceof Error?error.message:"disposition settlement failed"]});}});
  dispatch.on("group", "reopen-disposition", (ctx) => {if ((ctx.arg("--confirm")??"").trim().toLowerCase()!=="yes"){ctx.emitResult({ok:false,errors:["--confirm yes required to reopen intercompany disposition"]});return;}try{const workspace=resolveWorkspaceRoot(required(ctx,"--workspace")),audit=actorInput(ctx),id=required(ctx,"--disposition-id"),reader=openWorkspaceControlReadOnlyDb(workspace),before=inspectIntercompanyDisposition(reader,id);reader.close();if(!before)throw new Error("disposition not found");assertCompanySlugsAuthorize(workspace,[before.disposition.left.companySlug,before.disposition.right.companySlug],audit.createdBy,"group reopen-disposition");const db=openWorkspaceControlDb(workspace);try{ctx.emitResult({ok:true,...reopenIntercompanyDisposition(db,workspace,{dispositionId:id,payloadHash:required(ctx,"--payload-hash"),reason:required(ctx,"--reason"),actor:audit.createdBy,principal:{kind:"user",id:audit.createdBy}})});}finally{db.close();}}catch(error){ctx.emitResult({ok:false,errors:[error instanceof Error?error.message:"disposition reopen failed"]});}});
  dispatch.on("group", "supersede-disposition", (ctx) => {if ((ctx.arg("--confirm")??"").trim().toLowerCase()!=="yes"){ctx.emitResult({ok:false,errors:["--confirm yes required to supersede intercompany disposition"]});return;}try{const workspace=resolveWorkspaceRoot(required(ctx,"--workspace")),audit=actorInput(ctx),id=required(ctx,"--disposition-id"),reader=openWorkspaceControlReadOnlyDb(workspace),before=inspectIntercompanyDisposition(reader,id);reader.close();if(!before)throw new Error("disposition not found");assertCompanySlugsAuthorize(workspace,[before.disposition.left.companySlug,before.disposition.right.companySlug],audit.createdBy,"group supersede-disposition");const db=openWorkspaceControlDb(workspace);try{ctx.emitResult({ok:true,...supersedeIntercompanyDisposition(db,{dispositionId:id,payloadHash:required(ctx,"--payload-hash"),replacementDispositionId:required(ctx,"--replacement-disposition-id"),reason:required(ctx,"--reason"),actor:audit.createdBy,principal:{kind:"user",id:audit.createdBy}})});}finally{db.close();}}catch(error){ctx.emitResult({ok:false,errors:[error instanceof Error?error.message:"disposition supersession failed"]});}});

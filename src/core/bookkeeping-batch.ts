...
export function createBookkeepingBatchRun(db: Database, input: BookkeepingBatchPlan & BatchActor & { principal?: BatchPrincipal; runKey?: string }) {
  ...
  const stored = canonical(plan); if (hash({ companyId:plan.companyId, accountingFrom:plan.accountingFrom, accountingTo:plan.accountingTo, bankFrom:plan.bankFrom, bankTo:plan.bankTo, items:plan.items, sourceIdentities:plan.sourceIdentities, candidateSetHash:plan.candidateSetHash }) !== input.planHash) throw new Error("invalid canonical plan JSON or planHash");
...
export function approveBookkeepingBatchPlan(db: Database, input: BatchActor & { principal?: BatchPrincipal; runId: number; planHash: string }) { if (!validActor(input.actor)) throw new Error("actor is required"); const principal=batchPrincipal(db,input); return db.transaction(() => { const revision = db.query("SELECT id,planner_kind,planner_subject_id FROM bookkeeping_batch_revisions WHERE run_id=? AND revision_hash=?").get(input.runId,input.planHash) as {id:number;planner_kind:BatchPrincipal['kind'];planner_subject_id:string}|null; if (!revision) throw new Error("exact pending plan was not found"); if (revision.planner_kind===principal.kind && revision.planner_subject_id===principal.subjectId) throw new Error("SELF_APPROVAL_FORBIDDEN"); const existing=db.query("SELECT 1 FROM bookkeeping_batch_revision_approvals WHERE revision_id=?").get(revision.id); if (!existing) { db.query("INSERT INTO bookkeeping_batch_revision_approvals(revision_id,principal_kind,principal_subject_id,actor,created_at) VALUES(?,?,?,?,?)").run(revision.id,principal.kind,principal.subjectId,input.actor,new Date().toISOString()); event(db,input.runId,"approved",input.planHash,input.actor,{principal:{kind:principal.kind,subjectId:principal.subjectId}}); } return { ok: true as const }; }).immediate(); }
...
export function applyBookkeepingBatch(db: Database, input: BatchActor & { principal?: BatchPrincipal; runId: number; planHash: string; finalChecks?: Partial<Record<FinalCheckName, () => { ok: boolean; detail?: FinalCheckDetail }>> }) {
  ...
  const applyEvent=(type:"started"|"source_stale"|"item_applied"|"item_failed"|"final_checks"|"completed",detail:unknown,actionKey?:string)=>db.query("INSERT INTO bookkeeping_batch_apply_events_v2(apply_attempt_id,event_type,action_key,detail_json,created_at) VALUES(?,?,?,?,?)").run(attempt.id,type,actionKey??null,canonical(detail),new Date().toISOString());
  applyEvent("started",{candidateSetHash:plan.candidateSetHash});
  if (canonical(actualSources)!==canonical(plan.sourceIdentities) || hash((actualSources as any).candidateUniverse)!==revision.candidate_set_hash) {
  ...
  for (const [index,item] of plan.items.filter(x => x.partition === "ready").entries()) {
    ...
    try { db.query("INSERT INTO bookkeeping_batch_item_attempts(run_id,action_key,evidence_hash,outcome,created_at) VALUES(?,?,?,?,?)").run(input.runId,item.actionKey,item.evidenceHash,"started",new Date().toISOString()); const applied=applyPurchaseAction(db,plan,item,input.actor); if(applied.outcome!=="applied") throw new Error(applied.error); if(![applied.journalEntryId,applied.vatPreflightId,applied.postingRuleApplicationId,applied.purchaseApplicationId].every(Number.isInteger)) throw new Error("purchase application did not produce immutable evidence ids"); db.query("INSERT INTO bookkeeping_batch_applied_links(run_id,action_key,document_id,journal_entry_id,bank_transaction_id,vat_preflight_id,posting_rule_application_id,purchase_application_id,evidence_hash,created_at) VALUES(?,?,?,?,?,?,?,?,?,?)").run(input.runId,item.actionKey,applied.documentId,applied.journalEntryId,applied.bankTransactionId,applied.vatPreflightId,applied.postingRuleApplicationId,applied.purchaseApplicationId,item.evidenceHash,new Date().toISOString()); db.query("INSERT INTO bookkeeping_batch_item_receipts(run_id,action_key,receipt_json,created_at) VALUES(?,?,?,?)").run(input.runId,item.actionKey,canonical(applied),new Date().toISOString()); db.query("INSERT INTO bookkeeping_batch_item_attempts(run_id,action_key,evidence_hash,outcome,created_at) VALUES(?,?,?,?,?)").run(input.runId,item.actionKey,item.evidenceHash,"applied",new Date().toISOString()); db.exec(`RELEASE ${savepoint}`); applyEvent("item_applied",applied,item.actionKey); results.push({actionKey:item.actionKey,outcome:"applied"}); }
  ...
  const checks = (Object.keys(defaults) as FinalCheckName[]).map(name => { const checked=(input.finalChecks?.[name]??defaults[name])(); db.query("INSERT INTO bookkeeping_batch_final_checks_v2(apply_attempt_id,check_name,ok,detail_json,created_at) VALUES(?,?,?,?,?)").run(attempt.id,name,checked.ok?1:0,canonical(checked.detail??{}),new Date().toISOString()); return {name,...checked}; });

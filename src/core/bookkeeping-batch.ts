...
import { applyPostingRuleEvaluation, evaluatePostingRules, type PostingRuleContext } from "./posting-rules";
import { applyStoredPurchaseVatPreflight } from "./purchase-vat-preflight";
import { bookExpenseFromBank } from "./expense-booking";
...
export type BookkeepingBatchPlan = { companyId: number; accountingFrom: string; accountingTo: string; bankFrom: string; bankTo: string; items: BookkeepingBatchItem[]; planHash: string };
...
export function planBookkeepingBatch(db: Database, input: Omit<BookkeepingBatchPlan, "items" | "planHash">): BookkeepingBatchPlan {
  ...
  return { ...s, items, planHash: hash({ ...s, items }) };
}
...
export function createBookkeepingBatchRun(db: Database, input: BookkeepingBatchPlan & BatchActor & { runKey?: string }) {
  ...
  const fresh = planBookkeepingBatch(db, input); if (fresh.planHash !== input.planHash) throw new Error("planHash does not bind the supplied plan");
  return db.transaction(() => { const old = db.query("SELECT id,plan_hash,plan_json FROM bookkeeping_batch_runs WHERE run_key=?").get(runKey) as any; if (old) { if (old.plan_hash !== input.planHash) throw new Error("runKey already binds another plan"); return { runId: old.id, duplicate: true, plan: JSON.parse(old.plan_json) as BookkeepingBatchPlan }; } const row = db.query("INSERT INTO bookkeeping_batch_runs(run_key,company_id,accounting_from,accounting_to,bank_from,bank_to,plan_hash,plan_json,created_at) VALUES(?,?,?,?,?,?,?,?,?) RETURNING id").get(runKey, fresh.companyId, fresh.accountingFrom, fresh.accountingTo, fresh.bankFrom, fresh.bankTo, fresh.planHash, canonical(fresh), new Date().toISOString()) as { id: number }; event(db, row.id, "planned", fresh.planHash, input.actor); return { runId: row.id, duplicate: false, plan: fresh }; }).immediate();
}
export function approveBookkeepingBatchPlan(db: Database, input: BatchActor & { runId: number; planHash: string }) { if (!validActor(input.actor)) throw new Error("actor is required"); return db.transaction(() => { const run = db.query("SELECT plan_hash FROM bookkeeping_batch_runs WHERE id=?").get(input.runId) as any; if (!run || run.plan_hash !== input.planHash) throw new Error("exact pending plan was not found"); if (!db.query("SELECT 1 FROM bookkeeping_batch_events WHERE run_id=? AND event_type='approved' AND plan_hash=?").get(input.runId, input.planHash)) event(db, input.runId, "approved", input.planHash, input.actor); return { ok: true as const }; }).immediate(); }
...
function applyPurchaseAction(db: Database, plan: BookkeepingBatchPlan, item: BookkeepingBatchItem, actor: string) {
  ...
  const rule = applyPostingRuleEvaluation(db, evidence.context, { applicationKey: `${item.actionKey}:rule`, at: evidence.document.invoice_date ?? plan.accountingTo });
  if (rule.decision !== "proposed" || !rule.applicationId || typeof rule.outcome.account !== "string") return { outcome: "blocked" as const, error: rule.reasons.join("; "), exceptionId: rule.exceptionId };
  const vat = applyStoredPurchaseVatPreflight(db, item.documentId, { actor });
  if (!vat.ok || !vat.vatPreflightId) return { outcome: "blocked" as const, error: vat.errors.join("; "), exceptionId: vat.exceptionId };
  const posted = bookExpenseFromBank(db, { documentId: item.documentId, bankTransactionId: item.bankTransactionId, expenseAccountNo: rule.outcome.account, vatTreatment: rule.outcome.vatTreatment as any, createdBy: actor, createdByProgram: "bookkeeping-batch" });

...
export type WorkbenchRow = { bankTransactionId:number; date:string; text:string; amount:number; currency:string; bankAccount:{id:number|null;name:string|null}; document:{id:number;quality:"matched";party:{id:string;name:string}|null;resolutionState:"resolved"|"internal_no_external_party"|"unresolved"}|null; proposed:{account:string|null;vatTreatment:string|null;dimensions:Array<{dimensionId:string;memberId:string;status:"active"|"inactive"|"missing"}>}; status:WorkbenchStatus; nextAction:string; drilldown:{documentId?:number;partyId?:string;bankTransactionId:number;bankAccountId?:number;runId?:number;journalEntryId?:number;periodClose:{from:string;to:string}}; sourceHash:string };

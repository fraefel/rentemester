...
export type WorkbenchStatus = "ready" | "suggestedMatch" | "missingDocument" | "partyUnresolved" | "accountingDecisionRequired" | "vatEvidenceRequired" | "stalePlan" | "applyFailed";
export type WorkbenchFilter = { from: string; to: string; status?: WorkbenchStatus; bankAccountId?: number; limit?: number; cursor?: number; search?: string };
export type WorkbenchRow = { bankTransactionId:number; date:string; text:string; amount:number; currency:string; bankAccount:{id:number|null;name:string|null}; document:{id:number;quality:"matched";party:string|null}|null; proposed:{account:string|null;vatTreatment:string|null}; status:WorkbenchStatus; nextAction:string; drilldown:{documentId?:number;runId?:number;periodClose?:{from:string;to:string}}; sourceHash:string };
...
export function buildBookkeepingWorkbench(db:Database,input:WorkbenchFilter){
  ...
  const required=["companies","opening_balances","bank_transactions","bank_journal_reconciliations","bookkeeping_batch_runs","bookkeeping_batch_revisions","bookkeeping_batch_item_attempts"];
  ...
  if(active&&active.candidate_set_hash!==plan.candidateSetHash)try{const saved=JSON.parse(active.plan_json) as {items?:BookkeepingBatchItem[]};if(!Array.isArray(saved.items))throw new Error("invalid plan items");for(const item of saved.items){const current=item.bankTransactionId?byBank.get(item.bankTransactionId):undefined;if(item.bankTransactionId&&current&&current.evidenceHash!==item.evidenceHash)staleBanks.add(item.bankTransactionId);}}catch{revisionStateReadable=false;}
  ...
  const base=db.query(`SELECT bt.id,bt.transaction_date,bt.text,bt.amount,bt.currency,bt.bank_account_id,ba.name AS bank_name
    ...
    AND (? IS NULL OR bt.transaction_date>=?) AND (? IS NULL OR bt.bank_account_id=?)
    ORDER BY bt.transaction_date,bt.id`).all(input.from,input.to,cut,cut,input.bankAccountId??null,input.bankAccountId??null) as Array<{id:number;transaction_date:string;text:string;amount:number;currency:string;bank_account_id:number|null;bank_name:string|null}>;
  const needle=input.search?.trim().toLowerCase();
  const rows:WorkbenchRow[]=[];
  for(const bank of base){const item=byBank.get(bank.id);let status:WorkbenchStatus="missingDocument";let document:WorkbenchRow["document"]=null;let proposed={account:null as string|null,vatTreatment:null as string|null};
    if(item?.documentId){const d=db.query("SELECT id,sender_name FROM documents WHERE id=?").get(item.documentId) as {id:number;sender_name:string|null}|null;document=d?{id:d.id,quality:"matched",party:d.sender_name}:null;const detail=item.detail as any;proposed={account:detail?.rule?.outcome?.account??null,vatTreatment:detail?.rule?.outcome?.vatTreatment??null};if(item.partition==="ready")status="ready";else if(detail?.vatReady===false)status="vatEvidenceRequired";else if(!d?.sender_name)status="partyUnresolved";else status="accountingDecisionRequired";
    ...
    const row:WorkbenchRow={bankTransactionId:bank.id,date:bank.transaction_date,text:bank.text,amount:Number(bank.amount),currency:bank.currency,bankAccount:{id:bank.bank_account_id,name:bank.bank_name},document,proposed,status,nextAction:next(status),drilldown:{...(document?{documentId:document.id}:{}),...(failedBanks.has(bank.id)?{runId:failedBanks.get(bank.id)!}:{}),periodClose:{from:input.from,to:input.to}},sourceHash:digest({bank,item,document,proposed,status})};
    if(!needle||`${row.text} ${row.document?.party??""} ${row.proposed.account??""}`.toLowerCase().includes(needle))rows.push(row);
  }
  const populationRows=rows;
  const filtered=input.status?populationRows.filter(row=>row.status===input.status):populationRows;

...
function proposedDiff(db:Database,facts:OwnershipFactInput[]) {
  ...
  // Existing facts are never ended automatically. Missing observations are a
  // reviewable conflict, because registry coverage may be incomplete.
  ...
  return {version:1,add,unchanged,conflicts,automaticEndings:[],note:"Registry observations never automatically end approved legal facts"};
}
...
function assertGraphIntegrity(db:Database,incoming:OwnershipFactInput[]){
  if(new Set(incoming.map(sha)).size!==incoming.length)throw new Error("ownership snapshot contains duplicate canonical facts");
  const approved=(db.query("SELECT canonical_fact FROM rm_ownership_facts WHERE review_state='approved'").all()as any[]).map(r=>JSON.parse(r.canonical_fact)as OwnershipFactInput);
  ...
  for(const asOf of boundaries){
    ...
    for(const row of activeFacts){if(row.economicBasisPoints!=null)totals.set(row.ownedCompanySlug,(totals.get(row.ownedCompanySlug)??0)+row.economicBasisPoints);}
...
export function proposeOwnershipSnapshot(db:Database,input:OwnershipSnapshotInput) {
  const facts=(input.facts??[]).map((f,i)=>fact(db,f,`facts[${i}]`)); if(!facts.length||facts.length>2048)throw new Error("facts must contain 1..2048 rows"); assertGraphIntegrity(db,facts);
  ...
  const canonicalFacts=[...facts].sort((a,b)=>sha(a).localeCompare(sha(b))); const snapshotHash=sha({source,observedAt,facts:canonicalFacts}), diff=proposedDiff(db,canonicalFacts), diffHash=sha(diff);
...
export function applyOwnershipSnapshot(db:Database,input:{snapshotId:string;snapshotHash:string;diffHash:string;actor:string;principal:OwnershipPrincipal;authorized:boolean}) { if(!input.authorized)throw new Error("ownership apply requires live narrow workspace permission");const id=text(input.snapshotId,"snapshotId",80), snapshot=inspectSnapshot(db,id);if(!snapshot)throw new Error("ownership snapshot not found");if(snapshot.snapshotHash!==input.snapshotHash||snapshot.diffHash!==input.diffHash)throw new Error("exact snapshot and diff hashes are required");if(state(db,id)==="applied")return {status:"unchanged" as const,snapshot};if(state(db,id)!=="approved")throw new Error("only an approved ownership snapshot can be applied");const actor=text(input.actor,"actor",160),pid=text(input.principal.id,"principal id",160);let applied=false;db.transaction(()=>{const fresh=inspectSnapshot(db,id)!;if(state(db,id)==="applied")return;if(fresh.snapshotHash!==input.snapshotHash||fresh.diffHash!==input.diffHash||state(db,id)!=="approved")throw new Error("ownership snapshot became stale before apply");for(const item of fresh.diff.add as OwnershipFactInput[]){const canonicalFact=canonical(item), factHash=sha(item);db.query("INSERT OR IGNORE INTO rm_ownership_facts(fact_id,snapshot_id,fact_hash,canonical_fact,owner_kind,owner_id,owned_company_slug,valid_from,valid_to_exclusive,economic_exact_bp,economic_min_bp,economic_max_bp,voting_bp,control_type,share_class,jurisdiction,review_state,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)").run(`ownership-fact-${factHash.slice(0,32)}`,id,factHash,canonicalFact,item.owner.kind,item.owner.kind==="company"?item.owner.companySlug:item.owner.partyId,item.ownedCompanySlug,item.validFrom,item.validToExclusive??null,item.economicBasisPoints??null,item.economicIntervalBasisPoints?.min??null,item.economicIntervalBasisPoints?.max??null,item.votingBasisPoints??null,item.controlType,item.shareClass??null,item.jurisdiction,"approved",new Date().toISOString());}db.query("INSERT INTO rm_ownership_snapshot_events(snapshot_id,event_type,actor,principal_kind,principal_id,created_at) VALUES(?,?,?,?,?,?)").run(id,"applied",actor,input.principal.kind,pid,new Date().toISOString());applied=true;})();return {status:applied?"applied" as const:"unchanged" as const,snapshot:inspectSnapshot(db,id)!}; }
export function queryOwnershipGraph(db:Database,input:{asOf:string;visibleCompanySlugs?:ReadonlySet<string>}) { const asOf=date(input.asOf,"asOf");const all=(db.query("SELECT canonical_fact FROM rm_ownership_facts WHERE review_state='approved' ORDER BY fact_hash").all()as any[]).map(r=>JSON.parse(r.canonical_fact)as OwnershipFactInput).filter(f=>active(f,asOf));const visible=input.visibleCompanySlugs;const facts=visible?all.filter(f=>visible.has(f.ownedCompanySlug)&&(f.owner.kind!=="company"||visible.has(f.owner.companySlug))):all;const hidden=!!visible&&facts.length!==all.length;return {asOf,facts,partial:hidden,consolidation:{eligible:false,reason:hidden?"hidden endpoints":all.some(f=>f.owner.kind!=="company"||f.economicBasisPoints==null||f.validToExclusive!=null)?"party, interval or non-exact ownership requires explicit supported scope":"existing v1 manifest remains authoritative"}}; }

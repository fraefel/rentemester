...
export function proposeOwnershipSnapshot(db:Database,input:OwnershipSnapshotInput) {
  const facts=(input.facts??[]).map((f,i)=>fact(db,f,`facts[${i}]`)); if(!facts.length||facts.length>2048)throw new Error("facts must contain 1..2048 rows");
...
export function reviewOwnershipSnapshot(db:Database,input:{snapshotId:string;decision:"approved"|"rejected";actor:string;principal:OwnershipPrincipal}) { const snapshot=inspectSnapshot(db,text(input.snapshotId,"snapshotId",80));if(!snapshot)throw new Error("ownership snapshot not found"); const current=state(db,snapshot.snapshotId);if(current!=="proposed"){if(current===input.decision)return snapshot;throw new Error("ownership snapshot has already been reviewed");}const actor=text(input.actor,"actor",160),principalId=text(input.principal.id,"principal id",160);db.query("INSERT INTO rm_ownership_snapshot_events(snapshot_id,event_type,actor,principal_kind,principal_id,created_at) VALUES(?,?,?,?,?,?)").run(snapshot.snapshotId,input.decision,actor,input.principal.kind,principalId,new Date().toISOString());return inspectSnapshot(db,snapshot.snapshotId)!; }

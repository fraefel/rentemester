...
export function inspectParty(db: Database, partyId:string) { const created=db.query("SELECT canonical_kind,canonical_payload FROM rm_party_events WHERE party_id=? AND event_type='created'").get(partyId) as {canonical_kind:PartyKind;canonical_payload:string}|null; if(!created)return null; const history=db.query("SELECT event_type,payload_hash,actor,created_at FROM rm_party_events WHERE party_id=? ORDER BY id").all(partyId); const roles=db.query("SELECT company_slug,role,defaults_json FROM rm_party_company_roles WHERE party_id=? ORDER BY company_slug,role").all(partyId); return {partyId,kind:created.canonical_kind,...JSON.parse(created.canonical_payload),roles:roles.map((r:any)=>({companySlug:r.company_slug,role:r.role,defaults:JSON.parse(r.defaults_json)})),history}; }
export function searchParties(db:Database,input:{query?:string; companySlugs?:ReadonlySet<string>; cursor?:number; limit?:number}={}) { const limit=Math.min(Math.max(input.limit??25,1),100), cursor=Math.max(input.cursor??0,0); const rows=db.query("SELECT party_id,canonical_kind,canonical_payload FROM rm_party_events WHERE event_type='created' ORDER BY party_id").all() as Array<{party_id:string;canonical_kind:PartyKind;canonical_payload:string}>; const query=input.query?.trim().toLowerCase(); const visible=rows.map(r=>inspectParty(db,r.party_id)!).filter(p=>!query||p.name.toLowerCase().includes(query)||p.aliases.some((a:string)=>a.toLowerCase().includes(query))).map(p=>input.companySlugs?{...p,roles:p.roles.filter((r:any)=>input.companySlugs!.has(r.companySlug)),history:p.history.filter((event:any)=>event.event_type!=="linked")} :p).filter(p=>!input.companySlugs||p.roles.length>0); return {rows:visible.slice(cursor,cursor+limit),count:visible.length,nextCursor:cursor+limit<visible.length?cursor+limit:null}; }

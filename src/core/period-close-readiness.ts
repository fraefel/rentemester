...
export function createPeriodCloseReadinessPacket(db: Database, input: { periodStart: string; periodEnd: string; cutoff?: string }): CloseReadinessPacket {
  ...
  const identities = { ledgerHead: (db.query("SELECT entry_hash FROM journal_entries ORDER BY id DESC LIMIT 1").get() as {entry_hash?:string}|null)?.entry_hash ?? "GENESIS", auditRows: (db.query("SELECT COUNT(*) AS n FROM audit_log").get() as {n:number}).n, migrations: (db.query("SELECT group_concat(id || ':' || checksum, ',') AS v FROM (SELECT id,checksum FROM schema_migrations ORDER BY id)").get() as {v:string|null}).v ?? "" };

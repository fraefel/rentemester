...
export type CommitmentOccurrenceMatchInput={commitmentId:string;occurrenceDate:string;evidence:{kind:CommitmentEvidenceKind;id:string;date:string;amount:number;currency:string;partyId?:string;documented?:boolean};confirm:boolean;actor?:string;principal?:string};
...
export function matchSupplierCommitmentOccurrence(db:Database,input:CommitmentOccurrenceMatchInput){
 if(!input.confirm)return {ok:false,errors:["CONFIRMATION_REQUIRED"]};if(!validText(input.actor,160)||!validText(input.principal,160))return {ok:false,errors:["ACTOR_AND_PRINCIPAL_REQUIRED"]};
 if(!isValidIsoDate(input.occurrenceDate)||!isValidIsoDate(input.evidence.date)||!validText(input.evidence.id,160)||!Number.isFinite(input.evidence.amount)||input.evidence.amount<0||!/^[A-Z]{3}$/.test(input.evidence.currency))return {ok:false,errors:["INVALID_OCCURRENCE_OR_EVIDENCE"]};
 ...
 const evidence=input.evidence;const existingOccurrence=db.query("SELECT payload_hash FROM supplier_commitment_occurrence_matches WHERE commitment_id=? AND occurrence_date=?").get(input.commitmentId,input.occurrenceDate) as any;
 ...
 const expectedWithHash={...expected,payloadHash:row.payload_hash};const payload={commitmentId:input.commitmentId,occurrenceDate:input.occurrenceDate,expected:expectedWithHash,evidence:{...evidence,partyId:evidence.partyId??null,documented:evidence.documented!==false}};const payloadHash=digest(payload);
...
export function commitmentVariance(expected:{date:string;amount:number;currency:string},evidence:CommitmentOccurrenceMatchInput["evidence"]){const currencyMatch=expected.currency===evidence.currency;return {amount:currencyMatch?evidence.amount-expected.amount:null,dateDays:Math.round((Date.parse(evidence.date+"T00:00:00Z")-Date.parse(expected.date+"T00:00:00Z"))/86400000),currency:{expected:expected.currency,actual:evidence.currency,status:currencyMatch?"same":"unsupported_fx"},party:evidence.partyId?"provided":"missing",documentation:evidence.documented===false?"missing":"present",comparability:currencyMatch?"native_currency":"unsupported_fx"};}

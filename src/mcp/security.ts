...
export const MCP_TOOL_PERMISSIONS: Readonly<Record<string, RoutePermission>> = Object.freeze(Object.fromEntries([
  ...
  ..."accounts_add accounts_role_confirm bank_account_update company_sync_cvr customer_create dimension_definition_create dimension_member_create documents_enrich documents_extract_invoice documents_set_company_context posting_rule_propose recurring_invoice_create vendor_create".split(" ").map((n) => [n, "company.master-data"]),
  ...
  ..."documents_party_link_apply documents_party_link_supersede".split(" ").map((n) => [n, "company.master-data"]),
  ...
  ..."accrual_register asset_register bank_import bookkeeping_batch_dry_run bookkeeping_batch_persist budget_set dimension_assignment_apply supplier_commitment_apply supplier_commitment_change efaktura_konfigurer efaktura_modtag efaktura_modtag_workspace efaktura_onboard efaktura_registrer expense_book invoice_claim_compensation invoice_claim_interest invoice_credit_note invoice_issue invoice_render invoice_remind mileage_log payable_register recurring_invoice_generate recurring_invoice_run_workspace".split(" ").map((n) => [n, "company.draft.write"]),
  ...
  ..."bookkeeping_batch_approve dimension_assignment_supersede exception_resolve period_close period_close_review posting_rule_approve".split(" ").map((n) => [n, "company.review"]),

...
export const MCP_TOOL_PERMISSIONS: Readonly<Record<string, RoutePermission>> = Object.freeze(Object.fromEntries([
  ["agent_capability_search", "public.read"], ["agent_workflow_describe", "public.read"], ["cvr_lookup", "public.read"], ["customer_validate_vat", "public.read"], ["invoice_compensation_calc", "public.read"], ["invoice_interest_calc", "public.read"], ["invoice_interest_correction_calc", "public.read"], ["invoice_validate", "public.read"], ["meta_about", "public.read"], ["system_healthcheck", "public.read"],
  ["portfolio_overview", "workspace.read"], ["company_add", "workspace.manage"],
  ..."accounts_list accounts_roles_status accrual_register_report asset_register_report audit_log_list audit_verify bank_account_list bank_list bank_suggest_matches bookkeeping_batch_status budget_forecast budget_list budget_vs_actual company_profile_get customer_list efaktura_onboarding_status efaktura_status exceptions_list import_archive_list invoice_find invoice_list invoice_overdue invoice_status journal_dry_run journal_list mileage_list mileage_report payable_list period_close_readiness period_list posting_rule_explain reconcile_bank recurring_invoice_list retention_status tax_return_prepare vat_oss_report vat_report vendor_list".split(" ").map((n) => [n, "company.read"]),
  ...
  ..."expense_vat_preflight vat_eu_sales_list".split(" ").map((n) => [n, "company.external-lookup"]),
...
export type McpSecurityContext = { workspaceRoot: string; verify(): Promise<{ serviceAccountId: string } | null> };
...
export function createMcpSecurityContextFromEnv(env: NodeJS.ProcessEnv = process.env): McpSecurityContext | null {
  ...
  return {
    ...
    async verify() {
      ...
      try {
        const row = db.query(`SELECT "referenceId" AS user_id FROM "apikey" WHERE "key" = ? AND "configId" = ? AND COALESCE("enabled",1) = 1 AND ("expiresAt" IS NULL OR "expiresAt" > CURRENT_TIMESTAMP)`).get(hash, WORKSPACE_SERVICE_PRINCIPAL_CONFIG_ID) as { user_id?: string } | null;
        if (!row?.user_id) return null;
        const principal = db.query("SELECT 1 FROM rm_workspace_service_principals WHERE user_id = ?").get(row.user_id);
        return principal ? { serviceAccountId: row.user_id } : null;
      } finally { db.close(); }
    },
  };
}
...
export async function authorizeMcpTool(context: McpSecurityContext, name: string, args: Record<string, unknown>): Promise<{ root?: string } | null> {
  ...
  try {
    if (permission.startsWith("workspace.")) return authorizeWorkspaceRoute(db, context.workspaceRoot, { userId: principal.serviceAccountId, permission }).allowed ? {} : null;

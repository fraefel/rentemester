...
export const MCP_TOOL_PERMISSIONS: Readonly<Record<string, RoutePermission>> = Object.freeze(Object.fromEntries([
  ...
  ..."accounts_list accounts_roles_status accrual_register_report asset_register_report audit_log_list audit_verify bank_account_list bank_list bank_suggest_matches bank_reconciliation_correction_plan direct_bank_purchase_payable_correction_plan bookkeeping_batch_plan bookkeeping_batch_status bookkeeping_workbench budget_forecast liquidity_forecast_13_week budget_list budget_vs_actual company_profile_get customer_list dimension_assignment_list dimension_assignment_plan dimension_budget_list dimension_budget_plan dimension_definition_list dimension_member_list efaktura_onboarding_status efaktura_status exceptions_list import_archive_list invoice_compensation_calc invoice_find invoice_imported_receivables invoice_imported_receivables_backfill_plan invoice_interest_calc invoice_interest_correction_calc invoice_list invoice_overdue invoice_status journal_dry_run journal_list mileage_list mileage_report payable_list supplier_commitment_plan supplier_commitment_list supplier_commitment_matches supplier_commitment_alerts period_close_readiness period_close_status period_list posting_rule_explain reconcile_bank recurring_invoice_list retention_status system_healthcheck tax_return_prepare vat_oss_report vat_report vendor_list".split(" ").map((n) => [n, "company.read"]),

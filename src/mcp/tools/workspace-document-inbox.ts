...
export function registerWorkspaceDocumentInboxTools(server: McpServer): void {
  ...
  server.registerTool("workspace_inbox_assign", { title:"Approve workspace inbox destination", description:"Explicitly approves one already-authorized legal entity. Requires actor attribution and confirm:true; retry reads the same approved assignment. It never routes from filename, hash or candidate order. write-reversible.", inputSchema:{company,sourceId:z.string().min(1),companySlug:z.string().min(1),confirm:confirmField}, outputSchema:envelopeShape, annotations:write }, direct(async (args) => {
  ...
  server.registerTool("workspace_inbox_complete", { title:"Complete workspace inbox handoff", description:"Hands an approved source exactly once to canonical company document ingestion. Requires actor attribution and confirm:true. On a lost response, use workspace_inbox_status before retrying. write-irreversible.", inputSchema:{company,sourceId:z.string().min(1),companySlug:z.string().min(1),confirm:confirmField}, outputSchema:envelopeShape, annotations:write }, direct(async (args) => {

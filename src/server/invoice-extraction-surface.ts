...
import { DocumentEvidenceError, snapshotRegisteredDocument } from "../core/document-storage";
import { extractInvoice, type InvoiceExtractor, inspectDocumentInvoiceExtraction, MAX_INVOICE_PDF_BYTES } from "../core/invoice-extraction";

export async function extractDocumentInvoice(db: Database, companyRoot: string, documentId: number, extractor: InvoiceExtractor, actor?: string) {
  ...
  let pdfBytes: Buffer;
  try {
    const snapshot = snapshotRegisteredDocument(db, companyRoot, documentId, MAX_INVOICE_PDF_BYTES);
    if (snapshot.mimeType !== "application/pdf") throw new DocumentEvidenceError("invalid_metadata");
    pdfBytes = snapshot.bytes;
  } catch (error) {
    if (error instanceof DocumentEvidenceError && error.reason === "hash_mismatch") {
      throw new Error("EXTRACTION_SNAPSHOT_HASH_MISMATCH");
    }
    if (error instanceof DocumentEvidenceError && (error.reason === "invalid_metadata" || error.reason === "invalid_path" || error.reason === "invalid_size")) {
      throw new Error("EXTRACTION_SNAPSHOT_INVALID");
    }
    throw new Error("EXTRACTION_SNAPSHOT_UNAVAILABLE");
  }

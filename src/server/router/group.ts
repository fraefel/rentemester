...
/** Workspace control writes must never open, migrate or backup-lock an
 * arbitrary company ledger.  They retain the same request hardening and
 * confirmation contract as company mutations; lifecycle ids are naturally
 * idempotent in core and require exact content hashes. */
async function withWorkspaceControlMutation(config:ServerConfig, request:Request, run:(body:any, principal:any, actor:string)=>any){
  assertLocalhostWriteAllowed(request,config);assertMutationOriginAllowed(request,config);assertMutationContentType(request);
  if(!config.requestPrincipal)throw ApiError.unauthorized("missing or invalid credentials");
  const body=await readJsonBody(request);if(body.confirm!==true)throw ApiError.badRequest("denne handling er irreversibel og kræver 'confirm: true'",{subcode:"CONFIRM_REQUIRED"});
  return run(body,config.requestPrincipal,dispositionPrincipal(config.requestPrincipal).kind==="service"?`service:${config.requestPrincipal.serviceAccountId}`:`user:${config.requestPrincipal.userId}`);
}

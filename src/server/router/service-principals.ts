...
export async function handleServicePrincipalCreate(config: ServerConfig, request: Request): Promise<Response> {
  ...
  try {
    const issued = await createWorkspaceServicePrincipal(db, runtime.auth, { displayName: requireString(body, "displayName"), actor });
...
export async function handleServicePrincipalRotate(config: ServerConfig, request: Request): Promise<Response> {
  ...
  const { db, actor } = requireOwner(config); const runtime = authRuntime(config);
  try {
    const issued = await rotateWorkspaceServiceCredential(db, runtime.auth, { serviceAccountId: requireString(body, "serviceAccountId"), credentialId: requireString(body, "credentialId"), actor });
...
export async function handleServicePrincipalRevoke(config: ServerConfig, request: Request): Promise<Response> {
  ...
  const { db, actor } = requireOwner(config); const runtime = authRuntime(config);
  try {
    await revokeWorkspaceServiceCredential(db, runtime.auth, { serviceAccountId: requireString(body, "serviceAccountId"), credentialId: requireString(body, "credentialId"), actor });

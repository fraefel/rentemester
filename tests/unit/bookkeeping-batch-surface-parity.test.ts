...
test("shared batch contract is deterministic, actor-confirmable and resumable", () => {
 ...
 const run=createBookkeepingBatchRun(db,{...dry,runKey:"surface-parity",actor:"agent:test"}); approveBookkeepingBatchPlan(db,{runId:run.runId,planHash:dry.planHash,actor:"user:reviewer"});

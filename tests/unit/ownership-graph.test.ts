...
describe("#576 party-aware ownership graph",()=>{
  ...
  test("makes dual-handle stale apply deterministic without duplicating facts or events", async()=>{
    ...
    try {const proposal=proposeOwnershipSnapshot(left,{snapshotId:"dual-apply",source:"synthetic",observedAt:"2026-02-01T00:00:00Z",actor:"user:maker",principal,facts:[{owner:{kind:"company",companySlug:"parent"},ownedCompanySlug:"child",validFrom:"2026-01-01",economicBasisPoints:10000,controlType:"equity",jurisdiction:"DK",evidenceRefs:["synthetic"]}]});reviewOwnershipSnapshot(left,{snapshotId:proposal.snapshotId,decision:"approved",actor:"user:reviewer",principal});const apply=(db:ReturnType<typeof openWorkspaceControlDb>)=>applyOwnershipSnapshot(db,{snapshotId:proposal.snapshotId,snapshotHash:proposal.snapshotHash,diffHash:proposal.diffHash,actor:"user:reviewer",principal,authorized:true}).status;const outcomes=await Promise.all([Promise.resolve().then(()=>apply(left)),Promise.resolve().then(()=>apply(right))]);expect(outcomes.sort()).toEqual(["applied","unchanged"]);expect(left.query("SELECT count(*) AS n FROM rm_ownership_facts").get()).toEqual({n:1});expect(left.query("SELECT count(*) AS n FROM rm_ownership_snapshot_events WHERE snapshot_id=? AND event_type='applied'").get(proposal.snapshotId)).toEqual({n:1});}finally{left.close();right.close();}
  });
});
